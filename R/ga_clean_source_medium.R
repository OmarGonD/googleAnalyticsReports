...
ga_clean_source_medium <- function(data, language = "es", remove_spam = FALSE) {
  ...
  for (i in 1:nrow(data)) {
    ...
    rpp.path <- paste(c("^rpp.*"
    ),
    collapse="|")

    larepublica.path <- paste(c("^larepublica.*"
    ),
    collapse="|")
    ...
    larepublica <- grepl(larepublica.path, data$source[i],
                            ignore.case = T)

    rpp <- grepl(rpp.path, data$source[i],
                         ignore.case = T)
